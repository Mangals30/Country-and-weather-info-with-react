...
export const getBackgroundImage = (current) => {
    let bgImage = ''
    let color = 'black'
                     if(current.weather_descriptions.toString().toLowerCase() === 'partly cloudy')
                      {
                      ...
                      if(current.weather_descriptions.toString().toLowerCase() === 'widespread dust')
                      {
                        bgImage = Dust
                      }
                      if(current.weather_descriptions.toString().toLowerCase().includes('rain') || current.weather_descriptions.toString().toLowerCase().includes('shower') || current.weather_descriptions.toString().toLowerCase().includes('drizzle'))
                      {
                        
                        bgImage = Rain
                      }
                      if(current.weather_descriptions.toString().toLowerCase()=== 'thunderstorm')
                      {
                        
                        bgImage = Thunderstorm
                        color = 'white'
                      }
                      if(current.weather_descriptions.toString().toLowerCase().includes('haze') || current.weather_descriptions.toString().toLowerCase()=== 'mist')
                      {
